...
**What the design must absorb**

**Quoting agent (#7)** — the pricing ladder is built on these unit prices: units are runtime-invisible (declared only as Python comments, absent from `inventory` and every helper), so unit handling must be a static design decision, not something inferred from data at runtime. The `product` category's 100× span ($0.02–$2.00) is an artifact of 15 different units sharing one column, so no price-based sanity check is possible there. A discount ladder must key on line/order **value**, not raw quantity — quantity is incomparable across items (10,000 sheets vs. 200 table covers). `Rolls of banner paper (36-inch width)` at $2.50/roll is implausibly cheap and is simultaneously 28% of warehouse value. `search_quote_history` is required by the rubric but its `total_amount` values must never feed price computation — 5 of 100 rows are persisted `-1`/"Error parsing response." rows, and only 57 of the remaining 95 explanations' own stated totals reconcile with the total_amount recorded (errors run up to ~10,000×); use it for tone/precedent text only, filtered to `total_amount > 0`. `get_supplier_delivery_date`'s quantity buckets are unit-blind, so sheet-denominated orders (thousands of sheets) almost always land in the 7-day bucket — the customer-facing prose must not present that as a considered supply judgement.

**Inventory agent (#6)** — owns `ITEM_AMBIGUOUS` and `UNIT_NOT_UNDERSTOOD`. Resolution/ambiguity adjudication must run against the **carried** (seed-137, 18-item) catalogue, not the full 46-item universe — 7 of 8 near-collision name clusters found in the full catalogue (poster, cardstock, cups, coloured, decorative, notes, cards) dissolve once coverage is applied, leaving exactly one live collision: `Banner paper` ($0.30/sheet, category `paper`) vs. `Rolls of banner paper (36-inch width)` ($2.50/roll, category `large_format`), an 8.3× spread that a wrong resolution turns into an order-of-magnitude quoting error. `UNIT_NOT_UNDERSTOOD` should be scoped **partial/line-level, not fatal**: only 3 of the 20 sample requests use an unpriceable unit (reams ×2, "packets" ×1), always alongside other resolvable sheet-denominated lines, so a fatal refusal would needlessly kill quotable lines. A2/A3/A5 paper sizes are a *distinct* resolution failure from unit or name mismatches — A3 alone appears in 7 of 20 sample requests and has no catalogue entry at all — and needs its own explicit blocker decision (substitute silently vs. refuse) for consistency with the ream-refusal precedent. `get_all_inventory`'s `HAVING stock > 0` filter collapses "sold to zero" and "never carried" into the same empty result, so carriage must be established from the static `inventory` table, not inferred from that helper's absence.

**Replenishment agent (#9)** — reads `min_stock_level`, which is unit-blind and price-blind: it is drawn `U[50,150)` per item regardless of whether the item is priced per sheet, per roll, or per pad. Under seed 137, `A4 paper` (per sheet) gets `min_stock_level = 135` against `current_stock = 272` — roughly half a ream — while `Table covers` (per cover) gets 77 and `Large poster paper (24x36 inches)` gets 89 physical large-format sheets, an implausibly large physical reorder point for a bulky format. The threshold-triggered restock cost spans 31× across the carried catalogue ($5.20 for crepe paper to $162.50 for table covers) for no policy reason. `min_stock_level` is usable only as a boolean low-stock tripwire, never as a reorder quantity or economic signal. Given real sample order sizes (median 300 sheets, 75th percentile 1,000, max 10,000) dwarf every carried item's stock ceiling (793), `INSUFFICIENT_STOCK` is not an edge case here — it is the modal outcome for paper lines (~40% of sample order lines exceed the largest carried stock), so replenishment is the main execution path, not a rarely-hit branch. Separately, the cash guard (`CASH_INSUFFICIENT`) is essentially unreachable through normal restocking: the entire carried warehouse is worth $4,940 against a $45,060 opening cash balance, so it can only fire on a unit-confusion-driven order, not a genuine cash shortfall.

---

## Reference table: full abnormality list
