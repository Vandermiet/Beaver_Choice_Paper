...
the models can call is missing, or if one of the seven required helpers is not
visibly wrapped — so the drawing cannot quietly drift away from the system.
...
answer nothing else; the **systems of record** are touched only by the tools
that wrap a provided helper.

A tool box reads: the function's name, what it is for, and the starter helper it
wraps. Boxes drawn with a dashed border are **not offered to the model** — their
arguments are money or exact names, and they are called by the agent's output
function with what the arithmetic produced, so a row the books depend on can
never go unwritten because a model forgot to ask for it.
...
flowchart TB
    ...
    subgraph ORCHESTRATOR["ORCHESTRATOR — plans, routes, writes the letter. Never acts on a system of record."]
        ...
        orchestrator_agent["orchestrator_agent<br/>reads the enquiry into catalogue-blind requested lines,<br/>drives the sequence, derives the outcome, writes the prose"]
    ...
    subgraph INVENTORY["INVENTORY — does a requested line resolve to something we carry, and how short of it are we? Owns the five resolution blockers; never raises insufficient_stock."]
        ...
        shortlist_candidates["shortlist_candidates<br/>the four resolution rules over the catalogue:<br/>finish/material selects, colour/quality drops,<br/>the size veto, the category guard"]
        check_stock["check_stock<br/>stock for a resolved item, to compute the shortfall<br/>wraps get_stock_level"]
        resolve_requested_line["resolve_requested_line<br/>the verdict the envelope is built from<br/>rules only, no helper"]
    ...
    subgraph QUOTING["QUOTING — what does a resolved line cost? Prices every resolved line blind to stock. Owns unpriceable."]
        ...
        price_line["price_line<br/>the discount ladder on units, per line, never compounding<br/>arithmetic only, no helper"]
        check_against_precedent["check_against_precedent<br/>notes where our price sits against what history charged;<br/>a note only — precedent has no argument that could move a price"]
        record_quote["record_quote<br/>the quote registry: one row per priced line,<br/>written whether or not it becomes a sale"]
    end
    ...
    subgraph SALES["SALES — can we fill this priced line right now, and what did it cost the business? Owns insufficient_stock, exclusively. Owns no deadline."]
        ...
        verify_lines["verify_lines<br/>every line decided against one reading<br/>before any line is written"]
        record_sale["record_sale<br/>one transactions row per committed line, at the line total<br/>wraps create_transaction, under the shared lock"]
        read_cash["read_cash<br/>the post-write read whose delta is the cash evidence<br/>wraps get_cash_balance"]
        promised_date["promised_date<br/>from stock: the request date;<br/>restocked: the date replenishment bought"]
    end

    subgraph REPLENISHMENT["REPLENISHMENT — can we buy in what a customer was refused for want of stock, in time and within our means? Owns deadline_unmeetable and cash_insufficient."]
        reorder_thresholds["reorder_thresholds<br/>the stock floor to rebuild to;<br/>the only read of inventory.min_stock_level anywhere,<br/>because no required helper exposes it"]
        ...
        plan_restocks["plan_restocks<br/>order_qty = shortfall + min_stock_level, priced at<br/>supplier_cost_ratio, drawn per item from its name"]
        decide_restocks["decide_restocks<br/>the deadline guard, then the cash guard:<br/>per item, cheapest first, all-or-nothing within an item"]
        record_restock["record_restock<br/>money out, stock in, booked on the request date<br/>wraps create_transaction, under the shared lock"]
    end
    ...
    subgraph RECORDS["SYSTEMS OF RECORD AND THE TRAIL — munder_difflin.db"]
        transactions["transactions<br/>money and stock movements"]
        inventory_table["inventory<br/>the carried catalogue and its floors"]
        quotes_table["quotes / quote_requests<br/>the seeded quote history"]
        audit_trail["the audit trail<br/>agent_steps, blocker_signals, transaction_links,<br/>suspended_flows, quote_registry, quote_fulfilments<br/>+ a JSONL transcript sidecar"]
    end
    ...
    record_sale -->|"beaver.ledger: one lock, one row, one link"| create_transaction
    read_cash --> get_cash_balance
    cash_available --> get_cash_balance
    restock_arrival_date --> get_supplier_delivery_date
    record_restock -->|"beaver.ledger: one lock, one row, one link"| create_transaction
    ...
    class resolve_requested_line,read_stock,price_line,check_against_precedent,record_quote,verify_lines,record_sale,read_cash,promised_date,plan_restocks,decide_restocks,record_restock,reorder_thresholds unexposed
```

`reorder_thresholds` is drawn dashed and solid at once in the code's terms: it
is registered as a model-callable tool *and* called again by replenishment's
output function, because the reading a purchase is decided on is the one taken
in the same breath as the write. `snapshot_financials` and `cash_available` are
the same shape. The tool call is what the trail records; the output function's
call is what the money moves on, and they are the same function so the two
cannot drift apart.
...
sequenceDiagram
    ...
    alt nothing resolved
        ...
        sales-->>seam: AgentResponse: committed + declined, cash before/after
        ...
        opt pass-1 insufficient_stock declines ∩ inventory's restock needs
            ...
            Note over replenishment: order_qty = shortfall + min_stock_level —<br/>deadline guard first, then the cash guard —<br/>booked on the request date so pass 2 can see it
            ...
            opt anything was actually bought
                orchestrator->>seam: place_order → _commit, pass 2 — the declined lines only
                seam->>sales: run
...
- `cash_after_sale` is called `read_cash`, and is likewise called by sales'
  ...
  measures.
- The design's unnamed *(min-stock read)* became `reorder_thresholds`, which is
  model-callable. It reads `min_stock_level` and `unit_price` and deliberately
  never `current_stock`: `init_database` writes the `inventory` table once and
  nothing updates it at runtime, so that column still reads its seeded figure
  after we have sold the shelf bare. Live stock exists only as a sum over
  `transactions`.
