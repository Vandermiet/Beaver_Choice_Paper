...
- `snapshot_financials` wraps `generate_financial_report` because its
  ...
  restock landed. It is called twice on a commitment, and deliberately: the
  model's call is what the audit trail records, and the output function's call,
  taken in the same breath as the writes, is what the order is decided on. They
  are the same function rather than a tool and a shadow of it, so the reading
  the trail shows and the reading the money moved on cannot drift apart.
...
There is no `order_total` here. What an order comes to is quoting's
`quote_total` over the lines that committed — sales prices nothing, so a second
function summing the same column would be a second place for the total to be
    ...
    async with _write_lock:
        for line in lines:
            ...
            await asyncio.to_thread(
                _link_transaction, line, rowid, run_id, request_id, step_id
            )
...
def _link_transaction(
    ...
    """Join one written row back to the request, and to the quote it fulfils.
    ...
    row, and in the same breath as the row itself because a sale nobody can
    trace is worse than no sale at all.

    Args:
        line: The committed line.
        rowid: The `transactions` rowid it was written at.
        run_id: The run the sale belongs to.
        request_id: The request the sale belongs to.
        step_id: The delegation that committed it.
    """
    with starter.engine().begin() as conn:
        ...
        conn.execute(
            ...
            {"transaction_rowid": rowid, "quote_line_id": line.quote_line_id},
        )
